		...
		Search) 
			...
			case $engine in
				...
				Searx) choice=$(dmenu -i -p "Searx")
					[[ -n $choice ]] && link="https://searx.bar/search?q=""$choice" || exit ; break ;;
				...
				startpage) choice=$(dmenu -i -p " startpage") 
					[[ -n $choice ]] && link="https://startpage.com/?q=""$choice" || exit ; break ;;
				Back) ;;
				*) link="$bookmark" ; break ;;
			esac ;; 
		Switch)  newbrowser=$(printf "brave\\nicecat\\nsurf\\nvimb\\nlibrewolf" | dmenu -i -p "Choose Browser") ;;
		*) 
			idx=$(grep -nF "$bookmark" $BOOKMARKS/bookmark_titles | cut -f1 -d:) 
			if [ "$idx" ]; then 
				link=$(sed "$idx"'!d' $BOOKMARKS/bookmarks) ; break
			else
				link="$bookmark" ; break
			fi ;; 
	esac
