#!/bin/sh
while true; do 
	bookmark=$(printf "Bookmarks\\nSwitch Browser\\nYouTube\\nSearx\\nbol" | dmenu -h 40 -i) ; [[ -n $bookmark ]] || exit 
	case $bookmark in
		YouTube) choice=$(dmenu -h 40 -i -p "Youtube") ; [[ -n $choice ]] && link="https://www.youtube.com/results?search_query=""$choice" || exit ; break ;;
		...
		Bookmarks) 
			if [ -s $BOOKMARKS/bookmark_titles ]; then 
				...
				link=$(dmenu -h 40 -p "Search/URL") ; break
			fi ;;
		*) link="$bookmark" ; break ;;
	esac
done
